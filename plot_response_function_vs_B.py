...
file_to_open = data_folder / "Response_kernel_vs_B_mu=-39_L=100_Gamma=0.05_Omega=0_Lambda=0.56_B_in_(0-0.6).npz"
...
fig, ax = plt.subplots()
ax.plot(B_values**2/Delta, np.sqrt(K[:, 0, 0]/K[0, 0, 0]), "-o",  label=r"$K^{(L)}_{xx}(\Omega=$"+f"{Omega}"+r"$,\mu=$"+f"{np.round(mu,2)}"+r", $\lambda=$"+f"{Lambda})")
ax.plot(B_values**2/Delta, np.sqrt(K[:, 1, 0]/K[0, 0, 0]), "-o",  label=r"$K^{(L)}_{yy}(\Omega=$"+f"{Omega}"+r"$,\mu=$"+f"{np.round(mu,2)}"+r", $\lambda=$"+f"{Lambda})")
...
#%%
file_to_open = data_folder / "Response_kernel_vs_B_mu=-39_L=100_Gamma=0.2_Omega=0_Lambda=0.56_B_in_(0-0.6).npz"
Data = np.load(file_to_open)
